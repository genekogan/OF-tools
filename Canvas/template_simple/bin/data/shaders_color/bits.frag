...
void main( void ) {
	...
	float mz = 20.0*noise(vec3(p.x, 0.5, 1.5));
	p.y += 0.2*step(sin(mz*p.x + time), 0.25);
	p.y -= mod(p.y+0.01*time, grid.y);
	
	float z = noise(vec3(0.04*time, mult.x+ 1230.2*p.x, 90.2*mult.y*p.y));
	float z2 = smoothstep(z, 0.0, 0.04);
