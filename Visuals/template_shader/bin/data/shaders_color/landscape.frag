...
void main(void){
  ...
  vec3 U=vec3(0,1,0);//Camera Up Vector
	...
	float my =  sin(time/8.0);
	float camy = -1.0+my; 
	float tary = -1.0+my; 
  vec3 E=vec3(0., camy, speed);//vec3(-sin(time/4.0)*8.0,4,cos(time/4.0)*8.0); //Camera location; Change camera path position here
	E.y += 0.2;
  vec3 viewDir=vec3(E.x-sin(time*0.3),tary,E.z+1.0); //Change camere view vector here
  //vec3 E=vec3(2.0, 4, 4.0); //Camera location; Change camera path position here
  ...
  vec2 vPos=dir*gl_FragCoord.xy/resolution.xy - 3.0; // (2*Sx-1) where Sx = x in screen space (between 0 and 1)
  ...
  scp.z += 1.;
  scp.y -=0.25;
  E.y -= (terrain(E).x + terrain(E + vec3(0., 0., 0.1)).x + terrain(E + vec3(0., 0., -0.1)).x) / 7.;
	E.y += 2.;
  //Raymarching
  const vec3 e=vec3(0.1,0.03,0);
  const float MAX_DEPTH=50.0; //Max depth0.
  ...
  if (f<MAX_DEPTH){
    if (s.y==0.0)
      c=terrain_clr(p);
vec3 cc = vec3(1.8, c.g, c.b);
	 
	float m = smoothstep(3.0, 0.2, f);
	 c = mix(c,cc, 1.0-m);
