...
void main( void ) {
	...
	float dx = mod(p.x, 1.0/d);
	float dy = mod(p.y, 1.0/d);

	float z1 = p.x - dx;
	float z2 = p.y - dy;
	
	float z = noise(vec3(25.0*z1, 31.0*z2, time));
	...
	float zzz = 50.0+200.0*noise(vec3(17.3*z1, 29.3*z2, 0.2*time));
	float zz = 0.2+0.8*noise(vec3(zzz*p2.y, time, z));
	
	gl_FragColor = vec4(vec3(p.x*p.y*zz), 1.0);
}