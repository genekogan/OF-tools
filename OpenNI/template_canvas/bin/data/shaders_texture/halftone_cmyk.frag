...
//
// 2DRect simplex noise by Ian McEwan, distributed under
...
uniform sampler2DRect texture; // The image to reproduce as a halftone
...
vec4 texture2DRect_bilinear(sampler2DRect tex, vec2 st, vec2 dims, vec2 one) {
  vec2 uv = st * dims;
  vec2 uv00 = floor(uv - vec2(0.5)); // Lower left corner of lower left texel
  vec2 uvlerp = uv - uv00 - vec2(0.5); // Texel-local lerp blends [0,1]
  vec2 st00 = (uv00 + vec2(0.5)) * one;
  vec4 texel00 = texture2DRect(tex, st00);
  vec4 texel10 = texture2DRect(tex, st00 + vec2(one.x, 0.0));
  vec4 texel01 = texture2DRect(tex, st00 + vec2(0.0, one.y));
  vec4 texel11 = texture2DRect(tex, st00 + one);
  vec4 texel0 = mix(texel00, texel01, uvlerp.y); 
  vec4 texel1 = mix(texel10, texel11, uvlerp.y); 
  return mix(texel0, texel1, uvlerp.x);
}
 
// 2DRect simplex noise
 
// Description : Array- and textureless GLSL 2DRect simplex noise.
...
void main() {
	...
	vec3 texcolor = texture2DRect(texture, gl_FragCoord.st).rgb;
